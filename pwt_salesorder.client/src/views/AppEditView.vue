...
const saveOrder = async () => {
  const result = await fetchWrapper.put('/api/Sales', newData.value) as IResult<IOrder>;
  if (result.status){
    toast.add({ severity: 'success', summary: 'Info', detail: 'Data berhasil disimpan', life: 3000 });
    goBack();
  } else {
    toast.add({ severity: 'error', summary: 'Info', detail: `Data gagal disimpan. Pesan : ${result.message}`, life: 3000 });
  }
}
...
onMounted(async () => {
  const result = await fetchWrapper.get(`/api/Sales/${route.params.id}`)
  if (result.status){
    newData.value = result.data;
    newData.value.date = new Date(result.data.date)
  } else {
    toast.add({ severity: 'error', summary: 'Info', detail: `Data gagal ditemukan. Pesan : ${result.message}`, life: 3000 });
    goBack();
  }
})
</script>
<template>
  <Card>
    <template #title>Sales Order - Edit</template>
    <template #content>
